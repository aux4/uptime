...
```bash
aux4 uptime add --url <url> [--name <name>] [--expectedStatus <spec>] [--expectBody <spec>] [--header "Name: Value"]... [--configFile <file>] [--config <profile>]
...
aux4 uptime add --url https://api.example.com/health --name api-auth --header "Authorization: Bearer $TOKEN"
